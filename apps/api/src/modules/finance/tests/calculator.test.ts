...
describe('Finance Calculator Domain', () => {
  ...
  describe('calculateLoanStructure', () => {
    ...
    it('caps Micro Finance at â‚¹1.25L and exposes shortfall', () => {
      const result = calculateLoanStructure(
    ...
    it('calculates Term Loan 90% without capping', () => {
      const result = calculateLoanStructure(
        { availableMarginCapital: 100000 }, // project = 10,00,000
        { schemeCode: 'TERM', schemeName: 'Term', financingPercentage: 90, maxLoanAmount: 4500000 }
      );
      expect(result.loanAmount.toNumber()).toBe(900000); // 90% of 10L
      expect(result.requiredOwnContribution.toNumber()).toBe(100000);
      expect(result.shortfall.toNumber()).toBe(0);
    });

    it('caps Term Loan at â‚¹45L and exposes shortfall', () => {
      const result = calculateLoanStructure(
        { availableMarginCapital: 510000 }, // project = 51,00,000
        { schemeCode: 'TERM', schemeName: 'Term', financingPercentage: 90, maxLoanAmount: 4500000 }
      );
      // 90% of 51L = 45.9L. Capped at 45L.
      expect(result.loanAmount.toNumber()).toBe(4500000);
      expect(result.requiredOwnContribution.toNumber()).toBe(600000); // 51L - 45L
      expect(result.availableMarginCapital.toNumber()).toBe(510000);
      expect(result.shortfall.toNumber()).toBe(90000); // 6L - 5.1L
    });

    it('throws if project cost is below minimum', () => {
      expect(() =>
        calculateLoanStructure(
          { availableMarginCapital: 1000 },
          { schemeCode: 'A', schemeName: 'A', minProjectCost: 20000 }
        )
      ).toThrowError(/minimum/);
    });

    it('throws if project cost is above maximum', () => {
      expect(() =>
        calculateLoanStructure(
          { availableMarginCapital: 3000 },
          { schemeCode: 'A', schemeName: 'A', maxProjectCost: 20000 }
        )
      ).toThrowError(/maximum/);
    });
  });

  describe('getPeriodsPerYear', () => {
    it('throws error on unsupported frequency', () => {
       // @ts-expect-error Testing invalid runtime value
       expect(() => getPeriodsPerYear('WEEKLY')).toThrowError(/Unsupported payment frequency/);
    });
  });
